-- =====================================================
-- FIX: Update get_properties_func to handle plural/singular variations
...
BEGIN
    ...
    WHERE (
        ...
        OR LOWER(u.areaname) LIKE LOWER(area_name) || '%'
        OR LOWER(area_name) LIKE LOWER(u.areaname) || '%'
    )
...
-- Test the fix
SELECT 'Testing Dollar Hills property matching...' as status;
SELECT projectname, areaname, bhk, price_per_sft
FROM get_properties_func('Dollar Hills', NULL)
LIMIT 5;

SELECT '✅ Function updated! Now "Dollar Hills" will match "Dollar hill"' as status;
